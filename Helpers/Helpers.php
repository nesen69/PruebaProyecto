<?php
	...
	function headerIndex($data=""){
		$view_header = "Views/Template/header_index.php";
		require_once ($view_header);
	}

	function footerIndex($data=""){
		$view_footer = "Views/Template/footer_index.php";
		require_once ($view_footer);
	}	
	...
	function getFile(string $url, $data){
		...
		$file = ob_get_clean();//Limpia el buffer
		return $file;
	}
    ...
    function sendEmail($data,$template){
        if(ENVIRONMENT == 1){            
            ...
            $mail->Username   = 'nesen69@gmail.com';                     //SMTP username
            $mail->Password   = 'pepemusi_123';                               //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
            $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

            //Recipients
            $mail->setFrom('nesen69@gmail.com', 'Servidor Local');
   ...
   /* function sendMailLocal($data,$template){
	...
	function strClean($strCadena){
		/*$string = preg_replace(['/\s+/','/*\s|\s$/'],[' ',''], $strCadena); SUSTITUIDA POR LA SIGUIENTE*/ 
	...
	function passGenerator($length = 10){ // Funcion para generar contraseñas
		$pass = "";
		$longitudPass = $length;
