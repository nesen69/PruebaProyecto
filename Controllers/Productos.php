<?php 
	...
	class Productos extends Controllers{
		public function __construct(){
			...
			getPermisos(MPRODUCTOS); // Corresponde al numero en la BD/Tabla Modulo->idmodulo
		}
		...
		public function Productos(){
			...
			$data['page_tag'] = "Produtos - ".NOMBRE_EMPRESA;
			$data['page_title'] = "PRODUCTOS - <small>".NOMBRE_EMPRESA."</small>";
		...
		public function getProductos(){
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($arrData[$i]['stock'] > 0){
						$arrData[$i]['fecha_sorteo'] = "No aplica";
					}
					/*dep($arrData);
					die();*/
		...
		public function setProducto(){
			if($_POST){
				...
				if(empty($_POST['txtNombre']) || empty($_POST['txtCodigo']) || empty($_POST['listCategoria']) || empty($_POST['txtPrecio']) || empty($_POST['listStatus']) )
				{
					...
					$strDescripcionCorta = strClean($_POST['txtDescripcionCorta']);
					$request_producto = "";
		...
		public function delFile(){
			if($_POST){
				if(empty($_POST['idproducto']) || empty($_POST['file'])){
					...
					if($request_image){
						$deleteFile =  deleteFile($imgNombre);//Eliminar la foto de uploads
