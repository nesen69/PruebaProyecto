...
class Pedidos extends Controllers{
	...
	public function __construct(){
		...
		public function Pedidos(){
			...
			$data['page_title'] = "PEDIDOS - <small>".NOMBRE_EMPRESA."</small>";
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($_SESSION['permisosMod']['r']){
						...
						if($arrData[$i]['idtipopago'] == 1){
							$btnView .= ' <a title="Ver Transacción" href="'.base_url().'/pedidos/transaccion/'.$arrData[$i]['idtransaccionpaypal'].'" target="_blanck" class="btn btn-info btn-sm"><i class="fab fa-paypal" aria-hidden="true"></i></a>';
						}else{
							$btnView .= '<button class="btn btn-secondary btn-sm" disabled=""><i class="fab fa-paypal" aria-hidden="true"></i></button>';
						}
					}
		...
		public function orden($idpedido){
			...
			$data['page_tag'] = "Orden - ".NOMBRE_EMPRESA;
			$data['page_title'] = "ORDEN - <small>".NOMBRE_EMPRESA."</small>";
		...
		public function transaccion($transaccion){
			...
			$data['page_tag'] = "Detalle de la transacción";
			$data['page_title'] = "Detalle de la transacción - <small>".NOMBRE_EMPRESA."</small>";
		...
		public function getTransaccion(string $transaccion){
			if($_SESSION['permisosMod']['r'] and $_SESSION['userData']['idrol'] != RCLIENTES){//Los clientes no pueden hacer reembolsos
				if($transaccion == ""){
					...
					if(empty($requestTransaccion)){
						...
						$htmlModal = getFile("Template/Modal/modalReembolso",$requestTransaccion);
			...
			if($_SESSION['permisosMod']['u'] and $_SESSION['userData']['idrol'] != RCLIENTES){
				if($pedido == ""){
					...
					if(empty($requestPedido)){
						...
						$htmlModal = getFile("Template/Modal/modalPedido",$requestPedido);
						$arrResponse = array("status" => true, "html" => $htmlModal);
					}
				}
				echo json_encode($arrResponse,JSON_UNESCAPED_UNICODE);
