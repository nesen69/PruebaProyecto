<?php 
	...
	class Login extends Controllers{
		public function __construct(){
			...
			session_start();
			if(isset($_SESSION['login'])){ // Bloqueamos el acceso por barra de direcciones y se accedera solo con sesiones activas
			...
			if($_POST){
				if(empty($_POST['txtEmail']) || empty($_POST['txtPassword'])){
					...
					if(empty($requestUser)){
						$arrResponse = array('status' => false,  'msg' => 'El usuario ó contraseña son incorrectos.');
					} else {
						$arrData = $requestUser;
						if($arrData['status'] == 1){
