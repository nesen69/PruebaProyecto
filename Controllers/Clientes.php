<?php 
	...
	class Clientes extends Controllers{
		public function __construct(){
			...
			getPermisos(MCLIENTES);// Corresponde al numero en la BD/Tabla Modulo->idmodulo
		}
		...
		public function Clientes(){
			...
			$data['page_title'] = "CLIENTES - <small>".NOMBRE_EMPRESA."</small>";
			...
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']))
			{
				...
				if($request_user > 0 )
				{
					if($option == 1){
						...
						sendEmail($dataUsuario,'email_bienvenida');//Solo funcionara en el hosting
					}else{
