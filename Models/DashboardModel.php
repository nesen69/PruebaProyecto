<?php /*Todos los modelos deben crearse con la primera letra mayuscula*/
	...
	class DashboardModel extends Mysql
	{
		...
		public function selectVentasMes(int $anio, int $mes){
			...
			for ($i=0; $i < $dias ; $i++) { 
				...
				$sql = "SELECT DAY(fecha) AS dia, COUNT(idpedido) AS cantidad, SUM(monto) AS total 
						FROM pedido 
						WHERE DATE(fecha) = '$fechaVenta' AND status = 'Pagado' ".$where;
		...
		public function selectVentasAnio(int $anio){
			...
			$arrMeses = Meses();
			for ($i=1; $i <= 12; $i++) { 
				$arrData = array('anio' =>'','nro_mes'=>'','mes'=>'','venta'=>'');
				$sql = "SELECT $anio AS anio, $i AS mes, SUM(monto) AS venta 
				FROM pedido
				WHERE MONTH(fecha)= $i AND YEAR(fecha) = $anio AND status = 'Pagado'
