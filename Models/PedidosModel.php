	...
	class PedidosModel extends Mysql
	{
		...
		public function selectPedidos($idpersona = null){
			...
			$sql = "SELECT p.idpedido,
							...
							tp.idtipopago,
							p.status,
							CONCAT(per.nombres,' ',per.apellidos) as nombres
					FROM pedido p
		...
		public function reembolsoPaypal(string $idtransaccion, string $observacion){
			...
			if(!empty($requestData)){
				...
				//PARA SANDBOX - Pruebas locales
				/*if(isset($objData->purchase_units[0]->payments->captures[0]->links[1]->href)){
					$urlReembolso = $objData->purchase_units[0]->payments->captures[0]->links[1]->href;
				}else{
					$urlReembolso = "https://api.sandbox.paypal.com/v2/payments/captures/".$this->idTransaccion."/refund";
				}*/
